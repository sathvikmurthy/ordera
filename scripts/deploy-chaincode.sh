...
output=$(peer lifecycle chaincode install ${CHAINCODE_NAME}.tar.gz 2>&1) || {
    if echo "$output" | grep -q "already successfully installed"; then
        echo "⚠️  Chaincode already installed on Org1, skipping..."
    else
        echo "❌ Error: $output"
        exit 1
    fi
...
output=$(peer lifecycle chaincode install ${CHAINCODE_NAME}.tar.gz 2>&1) || {
    if echo "$output" | grep -q "already successfully installed"; then
        echo "⚠️  Chaincode already installed on Org2, skipping..."
    else
        echo "❌ Error: $output"
        exit 1
    fi
